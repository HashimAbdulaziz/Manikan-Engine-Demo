...
export default function ManikanWidget({ product, onClose }) {
  ...
  const recommendedSize = (() => {
    const userChestCirc = chest // user's chest circumference
    // Chest width is half the circumference of the garment
    let best = 'M'
  ...
  // Auto-select recommended size for returning users
  useEffect(() => {
    if (isReturningUser) {
  ...
  const generateTryOn = useCallback(async (size) => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        setError('Whoops! That t-shirt is fighting for its life! 😅 Please select a larger size for this body type.')
      } else {
  ...
  }, [sex, height, weight, chest, waist, hips, product.color_hex])
...
function GeneratingOverlay() {
  ...
  return (
    <div className="mw-generating">
      ...
      <p className="mw-gen-phase" key={phase}>{phases[phase]}</p>
